...
async function handleTextMessage(event) {
    ...
    } else if (text === 'ลงทะเบียน Admin' || text === 'ลงทะเบียน admin') {
        // ลงทะเบียน Admin (จป.วิชาชีพ)
        await handleAdminRegistration(event);
...
async function handleRegister(event) {
    ...
    const flexMessage = {
        ...
        contents: {
            ...
            contents: [
                ...
                createJSPBubble(),
                // Bubble 5: ปุ่ม จป.วิชาชีพ (Admin)
                createAdminBubble()
            ]
        }
    };
    ...
    await replyLineMessage(event.replyToken, [
        {
            type: 'text',
            text: '📋 กรุณาเลือกหน่วยงานของคุณ:\n\n👉 เลื่อนซ้าย-ขวา เพื่อดูหน่วยงานทั้งหมด\n👉 กดเลือก 1 หน่วยงาน เพื่อลงทะเบียนทันที\n\n💡 จป.เทคนิค/Admin: เลื่อนไปการ์ดสุดท้าย'
        },
        flexMessage
    ]);
}
...
// สร้าง Bubble สำหรับ จป.วิชาชีพ (Admin)
    ...
    return {
        ...
        body: {
            ...
            contents: [
                {
                    type: 'text',
                    text: 'รับการแจ้งเตือนทุกครั้งที่:',
                    size: 'sm',
                    color: '#666666'
                },
                {
                    type: 'text',
                    text: '• มีรายงานใหม่เข้ามา',
                    size: 'sm',
                    color: '#666666'
                },
                {
                    type: 'text',
                    text: '• มีรายงานรออนุมัติปิดงาน',
                    size: 'sm',
                    color: '#666666',
                    margin: 'none'
                },
                {
                    ...
                    action: {
                        type: 'message',
                        label: 'ลงทะเบียน Admin',
                        text: 'ลงทะเบียน Admin'
                    },
                    style: 'primary',
                    height: 'sm',
                    color: '#DC2626',
...
// Handle Admin Registration (จป.วิชาชีพ)
async function handleAdminRegistration(event) {
    ...
    await replyLineMessage(event.replyToken, [{
        type: 'text',
        text: `✅ ลงทะเบียน Admin สำเร็จ!\n\n👤 ชื่อ: ${profile.displayName}\n🔐 สิทธิ์: จป.วิชาชีพ (Admin)\n\n📬 คุณจะได้รับการแจ้งเตือน:\n  • เมื่อมีรายงานใหม่เข้ามา\n  • เมื่อมีรายงานรออนุมัติปิดงาน\n\n💡 เข้าระบบ Admin ได้ที่:\nhttps://unsafe-report-xi.vercel.app/`
    }]);
}

// Handle Unit Selection (เลือกทีละหน่วยงาน - รองรับ Toggle)
async function handleSelectUnitAndRegister(event, unitName) {
    const userId = event.source.userId;
    ...
    // ลงทะเบียนทันทีเมื่อเลือกหน่วยงาน (ไม่ต้องพิมพ์ "เสร็จสิ้น")
    const profile = await getLineProfile(userId);
...
3️⃣ "ยกเลิก" - ยกเลิกการลงทะเบียนเพื่อลงทะเบียนใหม่
4️⃣ "ช่วยเหลือ" - แสดงคู่มือการใช้งาน

💡 วิธีลงทะเบียน:
• พิมพ์ "ลงทะเบียน"
• กดเลือกหน่วยงานของคุณ 1 หน่วยงาน
• ระบบจะลงทะเบียนให้ทันที

👷 สำหรับ จป.เทคนิค:
• เลื่อนไปการ์ดก่อนสุดท้าย
• กดปุ่ม จป. ของ กฟส. ที่ดูแล

🔐 สำหรับ จป.วิชาชีพ (Admin):
• เลื่อนไปการ์ดสุดท้าย
• กดปุ่ม "ลงทะเบียน Admin"
• รับแจ้งเตือนทุกรายงานใหม่และรออนุมัติ
...
🌐 เว็บไซต์: https://unsafe-report-xi.vercel.app`;
...
module.exports = async (req, res) => {
    ...
    if (req.method === 'POST') {
        try {
            // Check if this is a sendMessage request from our app
            ...
            // Otherwise, handle LINE webhook events
